...
from . import mclass
from .mclass import func, re, Expression
...
# Expression class object to hold expression being evaluated

# Exp = mclass.Expression()
...
class InputHandler(Handler):
    ...
    def match(self, user_id=0):
        ...
        user_id = user_id or self.request.get('user_id') or users.addUser(Expression(q=q, n=1)) 	# Change this

        Exp = users.getUser(int(user_id))

        logging.info("Created user - %s", int(user_id))

        self.render("get_dimn.html", num=Exp.n if num else '', q=q, user_id=user_id,
...
"""
class MatrixChecker(InputHandler):
...
class MatrixHandler(Handler):
            ...
            for i in xrange(r):
                for j in xrange(c):
                    ...
                    if not (A[i][j] and fl.match(A[i][j])):
                        # logging.info(A[i][j])
                        error = error or "All cells need to be filled"
                    else:
                        A[i][j] = float(A[i][j])
            matrices.append(A)
    ...
    def calc(self, op, matrices):
        ...
        return result
    """
    def rendernew(self, template, **kw):
        q = self.request.get('q')
        num = self.request.get('num')

        kw['exp'] = self.Exp.getExp()
        logging.info('rendernew called')
        logging.info('Evaluate ' + repr(kw['exp']))
        if not self.test('1', '1', q=q, num=num):
            return
        num = int(num)
        logging.error(num)

        if self.Exp.dim == []:
            dim = self.find_dim(q, num)
        # if error:
        #    self.home()
        #    return

        self.Exp.dim = dim
        logging.error('get_mat about to be called')
        kw['error'], kw['matrices'] = self.get_mat(dim, num)
        kw['error'] = '' if kw.get('first') else kw['error']
        logging.error(kw['error'])
        if kw['error'] and kw.get('res'):
            self.redirect('/mathrix/input')
            err_page(self.render_str(template, q=q, num=num, dim=dim, page="result", **kw))
        elif not kw.get('res'):
            self.render(template, q=q, num=num, dim=dim, page="result", **kw)
        else:
            return kw['matrices']
"""
    def get(self):
...
class EvalHandler(Handler):
    ...
    def post(self):
        ...
        try:
            ...
            return self.redirect('/mathrix?q=evaluate&user_id=%s' % user_id)
        except Exception as inst:
...
class OutputHandler(Handler):
    ...
    def post(self):
        ...
        op = Exp.q
        logging.error('Operation %s to be executed' % op)
        soln, steps, error, span = None, None, None, None
        self.render("loading.html")

        try:
            if op == 'evaluate':
                logging.error('Expression being evaluated')
...
app = webapp2.WSGIApplication([
    ('/mathrix/result.*', OutputHandler),
    ('/mathrix/num.*', MultipleMatrices),
    ('/mathrix/input.*', MatrixHandler),
    ('/mathrix/evaluate.*', EvalHandler),
    ('/mathrix', MainHandler),
    ('/mathrix.+', InputHandler)
], debug=DEBUG)
